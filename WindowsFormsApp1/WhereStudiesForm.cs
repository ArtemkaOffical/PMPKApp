...
namespace PMPK
{
    public partial class WhereStudiesForm : Form, IForm
    {
        private UnitOfWork _unitOfWork;

        public WhereStudiesForm()
        {
            InitializeComponent();
        ...
        {
            _unitOfWork.Save();
            DataTable data = _unitOfWork.PlaceStydiesRepository.Get().ToList().ConvertToDataTable();
        ...
        {
            _unitOfWork.PlaceStydiesRepository.Insert(new PlaceStudy());
        ...
        public void Save()
        {
            for (int i = 0; i < dataGridView1.Rows.Count - 1; i++)
            {
                PlaceStudy currentClass = _unitOfWork.PlaceStydiesRepository.GetByID(Convert.ToInt32(dataGridView1["Id", dataGridView1.Rows[i].Index].Value));
        ...
        public void Delete()
        {
            if (dataGridView1.CurrentRow != null)
            {
                ...
                {
                    _unitOfWork.PlaceStydiesRepository.Delete(Convert.ToInt32(dataGridView1["Id", dataGridView1.CurrentRow.Index].Value));
                    UpdateDataGridView();
                }
            }
        }
        ...
        private void WhereStudiesForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Dispose();
        }
    }
